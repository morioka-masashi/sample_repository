...
-- アジア大陸の表面積の合計を表示してください。
    SELECT  SUM(`surface_area`) AS 'アジアの表面積の合計'
    FROM countries
    WHERE continent = 'Asia';
...
-- 全ての国と言語と市区町村を表示してください。
    SELECT  c.name AS '国名', m.name AS '市町村名', l.language
    FROM countries AS c 
    INNER JOIN cities AS m 
    ON c.code = m.country_code
    INNER JOIN countrylanguages AS l
    ON m.country_code = l.country_code;

-- ★問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
    SELECT  ce.name, co.name
...
-- 全ての有名人の名前,国名、第一言語を出力してください。
    SELECT  ce.name, co.name,cl.language
    FROM celebrities AS ce 
    INNER JOIN countries AS co 
    ON ce.country_code = co.code
    INNER JOIN countrylanguages AS cl 
...
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示　させてください。
    SELECT ce.country_code, MAX(ce.age),MIN(ce.age)
    FROM celebrities AS ce 
    INNER JOIN countries AS co 
    ON ce.country_code = co.code
...
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
    SELECT SUBSTRING(birth,1,4) AS '誕生年',COUNT(birth) AS '人数'
    FROM celebrities
    WHERE birth
    LIKE '1980%'
    UNION
    SELECT SUBSTRING(birth,1,4),COUNT(birth) AS '人数'
    FROM celebrities
    WHERE birth
    LIKE '1981%';
